...
def register_drc_tools(mcp: FastMCP) -> None:
    ...
    def run_drc_check(project_path: str) -> Dict[str, Any]:
        ...
        logging.info(f"Starting DRC check on {os.path.basename(pcb_file)}")

        # Run DRC using the appropriate approach
        drc_results = None

        print("Using kicad-cli for DRC")
        logging.info("Using KiCad CLI for DRC check...")
        ...
        if drc_results and drc_results.get("success", False):
            ...
            if comparison:
                ...
                if comparison["change"] < 0:
                    ...
                    logging.info(f"No change in the number of DRC violations since the last check.")
        elif drc_results:
