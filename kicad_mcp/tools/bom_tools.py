...
def register_bom_tools(mcp: FastMCP) -> None:
    ...
    def analyze_bom(project_path: str) -> Dict[str, Any]:
        ...
        logging.info(f"Looking for BOM files related to {os.path.basename(project_path)}")
        ...
        # Progress removed(100, 100)
        logging.info(f"BOM analysis complete: found {total_components} components")
    ...
    def export_bom_csv(project_path: str) -> Dict[str, Any]:
        ...
        # KiCad modules availability - set to False since we converted to CLI-based approach
        ...
        # Progress removed(20, 100)
        logging.info(f"Found schematic file: {os.path.basename(schematic_file)}")
        ...
        if kicad_modules_available:
            try:
                ...
                logging.info("Attempting to export BOM using KiCad Python modules...")
                export_result = export_bom_with_python(
                    schematic_file, project_dir, project_name
                )
            except Exception as e:
                print(f"Error exporting BOM with Python modules: {str(e)}", exc_info=True)
                logging.info(f"Error using Python modules: {str(e)}")
                export_result = {"success": False, "error": str(e)}

        # If Python method failed, try command-line method
        if not export_result.get("success", False):
            try:
                logging.info("Attempting to export BOM using command-line tools...")
                export_result = export_bom_with_cli(
                    schematic_file, project_dir, project_name
                )
            except Exception as e:
                print(f"Error exporting BOM with CLI: {str(e)}", exc_info=True)
                logging.info(f"Error using command-line tools: {str(e)}")
                export_result = {"success": False, "error": str(e)}
        ...
        if export_result.get("success", False):
            ...
            logging.info(f"Failed to export BOM: {export_result.get('error', 'Unknown error')}")
...
def export_bom_with_python(
    ...
    try:
        ...
        logging.info("BOM export with Python modules not fully implemented yet")
