...
def register_export_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    def generate_pcb_thumbnail(project_path: str):
        ...
        try:
            ...
            if "pcb" not in files:
                ...
                logging.info("PCB file not found in project")
                return None
            ...
            logging.info(f"Generating thumbnail for {os.path.basename(pcb_file)} using kicad-cli")

            # Use command-line tools
            try:
                thumbnail = generate_thumbnail_with_cli(pcb_file)
                if thumbnail:
                    print("Thumbnail generated successfully via CLI.")
                    return thumbnail
                else:
                    print("generate_thumbnail_with_cli returned None")
                    logging.info("Failed to generate thumbnail using kicad-cli.")
                    return None
            except Exception as e:
                print(f"Error calling generate_thumbnail_with_cli: {str(e)}", exc_info=True)
                logging.info(f"Error generating thumbnail with kicad-cli: {str(e)}")
                return None
    ...
    @mcp.tool()
    def generate_project_thumbnail(project_path: str):
...
def generate_thumbnail_with_cli(pcb_file: str):
    ...
    try:
        ...
        logging.info("Using KiCad command line tools for thumbnail generation")
        ...
        try:
            ...
            # Progress removed(90, 100)
            # Inform user about the saved file
            logging.info(f"Thumbnail saved to: {output_file}")
            ...
            logging.info(f"KiCad CLI command failed: {e.stderr or e.stdout}")
            return None
        except subprocess.TimeoutExpired:
            print(f"Command timed out after 30 seconds: {' '.join(cmd)}")
            logging.info("KiCad CLI command timed out")
            return None
        except Exception as e:
            print(f"Error running CLI command: {str(e)}", exc_info=True)
            logging.info(f"Error running KiCad CLI: {str(e)}")
            return None
